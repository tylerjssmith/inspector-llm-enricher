...
# --- handle_failure() --------------------------------------------------------
def test_handle_failure(valid_finding) -> None:
    finding_arn = valid_finding.get('findingArn')
    error = handle_failure(stage='Test', finding_arn = finding_arn)
...
# --- get_nested() ------------------------------------------------------------
def test_get_nested(valid_finding, field_schema) -> None:
...
def test_get_nested_missing_path(valid_finding) -> None:
    value = get_nested(valid_finding, ['nonexistent', 'path'])
    assert value is None


# --- normalize_finding() -----------------------------------------------------
def test_normalize_finding(valid_finding, field_schema) -> None:
...
def test_normalize_finding_field_missing(valid_finding, field_schema) -> None:
    del valid_finding['severity']
    finding = normalize_finding(valid_finding, field_schema)
    assert finding is None


def test_normalize_finding_field_not_string(valid_finding, field_schema) -> None:
    valid_finding['severity'] = 99
    finding = normalize_finding(valid_finding, field_schema)
    assert finding is None


def test_normalize_finding_field_invalid(valid_finding, field_schema) -> None:
...
# --- make_user_prompt() ------------------------------------------------------
def test_make_user_prompt(normalized_finding) -> None:
...
# --- make_email_subj() -------------------------------------------------------
def test_make_email_subj(normalized_finding) -> None:
...
def test_make_email_subj_max_length(normalized_finding) -> None:
    long_finding = {**normalized_finding, 'type': normalized_finding['type'] * 2}
    email_subj = make_email_subj(long_finding)
    assert isinstance(email_subj, str)
...
# --- make_email_body() -------------------------------------------------------
def test_make_email_body(normalized_finding) -> None:
